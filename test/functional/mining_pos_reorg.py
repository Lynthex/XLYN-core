...
from test_framework.test_framework import LynthexTestFramework
...
class ReorgStakeTest(LynthexTestFramework):
    ...
    def check_money_supply(self, expected_xlyn, expected_zxlyn):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected XLYN and zXLYN supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_xlyn))
            for denom in node['zXLYNsupply']:
                assert_equal(node['zXLYNsupply'][denom], DecimalAmt(expected_zxlyn[denom]))
    ...
    def run_test(self):
        ...
        # Check XLYN and zXLYN supply at the beginning
        # ------------------------------------------
        # zXLYN supply: 2 coins for each denomination
        expected_zxlyn_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # XLYN supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zxlyn_supply)
        ...
        # Verify that XLYN and zXLYN supplies were properly updated after the spends and reorgs
        self.log.info("Check XLYN and zXLYN supply...")
        ...
        expected_zxlyn_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zxlyn_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zxlyn_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zxlyn_supply)
