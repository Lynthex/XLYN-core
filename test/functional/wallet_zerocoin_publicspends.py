...
class ZerocoinSpendTest(LynthexTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zxlyn_bal, xlyn_bal):
            zxlyn_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zxlyn_bal)
            xlyn_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], xlyn_bal)
            return zxlyn_bal, xlyn_bal
        ...
        zxlyn_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zxlyn_balance, 6666)
        ...
        zxlyn_balance, balance = check_balances(denom_2, zxlyn_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zxlyn_balance, balance = check_balances(denom_3, zxlyn_balance, balance)
        ...
        _, _ = check_balances(denom_1, zxlyn_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
