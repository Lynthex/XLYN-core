...
{
    // double check that there are no double spent zXLYN spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zXLYN serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zXLYN spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zXLYN check to not include duplicated serials in the same block.
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    util::ThreadRename("lynthex-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
