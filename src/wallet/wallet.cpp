...
/**
 * Fees smaller than this (in uXLYN) are considered zero fee (for transaction creation)
        ...
        if (zwallet) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zXLYN seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZXLYN = GetBoolArg("-zapwallettxes", false);
    if (fCheckZXLYN)
        zxlynTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if zXLYN upgrade is active.
            doZXlynRescan(pindex, block, setAddedToWallet, consensus, fCheckZXLYN);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-XLYN-address
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in XLYN) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of LYNTHEX"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart LYNTHEX to complete"));
            return nullptr;
        } else {
    ...
    if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
        if (prev_version <= FEATURE_PRE_LYNTHEX && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_LYNTHEX);
        }
    ...
    CzXLYNWallet* zwalletInstance = new CzXLYNWallet(walletInstance);
    ...
    if (!zwalletInstance->GetMasterSeed().IsNull()) {
        //Inititalize zXLYNWallet
        uiInterface.InitMessage(_("Syncing zXLYN wallet..."));

        //Load zerocoin mint hashes to memory
        walletInstance->zxlynTracker->Init();
