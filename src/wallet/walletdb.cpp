...
void ThreadFlushWalletDB()
{
    ...
    util::ThreadRename("lynthex-wallet");
...
bool CWalletDB::WriteDeterministicMint(const CDeterministicMint& dMint)
{
    ...
    return Write(std::make_pair(std::string("dzxlyn"), hash), dMint, true);
}

bool CWalletDB::ReadDeterministicMint(const uint256& hashPubcoin, CDeterministicMint& dMint)
{
    return Read(std::make_pair(std::string("dzxlyn"), hashPubcoin), dMint);
}

bool CWalletDB::EraseDeterministicMint(const uint256& hashPubcoin)
{
    return Erase(std::make_pair(std::string("dzxlyn"), hashPubcoin));
}
...
bool CWalletDB::ArchiveDeterministicOrphan(const CDeterministicMint& dMint)
{
    ...
    if (!Erase(std::make_pair(std::string("dzxlyn"), dMint.GetPubcoinHash())))
...
bool CWalletDB::WriteZXLYNSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZXLYNSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZXLYNSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZXLYNSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZXLYNSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZXLYNSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZXLYNCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZXLYNCount(uint32_t& nCount)
{
...
std::list<CDeterministicMint> CWalletDB::ListDeterministicMints()
{
    ...
    for (;;)
    {
        ...
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(std::string("dzxlyn"), UINT256_ZERO);
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
