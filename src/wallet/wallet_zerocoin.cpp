    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZXLYNSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZXLYNSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zXLYNseed to wallet";
    }
    //the use case for this is no password set seed, mint dzXLYN,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZXLYNSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZXLYNSeed(hashSeed, vchSeed)) {
...
void CWallet::doZXlynRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZXLYN)
{
    //If this is a zapwallettx, need to read zXLYN
    if (fCheckZXLYN && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
...
bool CWallet::CreateZXLYNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwallet->GenerateDeterministicZXLYN(denomination, coin, dMint);
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZXLYNOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zXLYN output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to XLYN
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    int nStatus = ZXLYN_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZXLYN_WALLET_LOCKED);
        return false;
    }
    ...
    if (res.status != CWallet::CommitStatus::OK) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZXLYN_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZXLYN_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zxlynTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zxlynTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZXLYN_SPEND_OKAY);  // When we reach this point spending zXLYN was successful
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZXLYN_TXMINT_GENERAL);
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZXLYN_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZXLYN_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZXLYN_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZXLYNModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZXLYN_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
    ...
    receipt.SetStatus(_("Spend Valid"), ZXLYN_SPEND_OKAY); // Everything okay
        ...
        CTxDestination * changeAddress)
{
    ...
    int nStatus = ZXLYN_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zXLYN"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZXLYN_TRX_CREATE;
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zXLYN public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zXLYN used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zxlynTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zXLYN mints to use in this spend
    ...
    for (CZerocoinMint mint : vSelectedMints) {
        ...
        if (IsSerialInBlockchain(mint.GetSerialNumber(), nHeightSpend)) {
            ...
            if (!zxlynTracker->HasSerialHash(hashSerial))
                return error("%s: tracker does not have serialhash %s", __func__, hashSerial.GetHex());

            CMintMeta meta = zxlynTracker->Get(hashSerial);
            meta.isUsed = true;
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to XLYN address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            unsigned int nBytes = ::GetSerializeSize(txNew, SER_NETWORK, PROTOCOL_VERSION);
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZXLYN_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZXLYN_SPEND_OKAY); // Everything okay
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zXLYN in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zxlynTracker->GetMints(true);
    ...
    return zxlynTracker->GetBalance(false, false);
}

CAmount CWallet::GetUnconfirmedZerocoinBalance() const
{
    return zxlynTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zxlynTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzXLYNWallet* zwallet)
{
    this->zwallet = zwallet;
    zxlynTracker = std::unique_ptr<CzXLYNTracker>(new CzXLYNTracker(this));
}

CzXLYNWallet* CWallet::getZWallet()
{
    return zwallet;
}
...
{
    return zxlynTracker->HasSerial(bnSerial);
}

bool CWallet::IsMyMint(const CBigNum& bnValue) const
{
    if (zxlynTracker->HasPubcoin(bnValue))
        return true;
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zxlynTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zxlynTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zxlynTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zxlynTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zxlynTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zxlynTracker->UpdateState(meta);
    } else {
