...
#include "zxlyn/zxlynmodule.h"
#include "zxlyn/zxlynwallet.h"
#include "zxlyn/zxlyntracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_LYNTHEX = 61000, // inherited version..
...
// Possible states for zXLYN send
enum ZerocoinSpendStatus {
    ZXLYN_SPEND_OKAY = 0,                            // No error
    ZXLYN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXLYN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXLYN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXLYN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXLYN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXLYN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXLYN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXLYN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXLYN_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZXLYN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXLYN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXLYN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXLYN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXLYN_SPENT_USED_ZXLYN = 14,                      // Coin has already been spend
    ZXLYN_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZXLYN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void doZXlynRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZXLYN);
    ...
    bool CreateZXLYNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zXLYN wallet
    std::unique_ptr<CzXLYNTracker> zxlynTracker{nullptr};
    void setZWallet(CzXLYNWallet* zwallet);
    CzXLYNWallet* getZWallet();
