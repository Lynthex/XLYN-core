...
public:
    ...
    bool WriteZXLYNSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZXLYNSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZXLYNSeed_deprecated(uint256& seed);
    bool EraseZXLYNSeed();
    bool EraseZXLYNSeed_deprecated();

    bool WriteZXLYNCount(const uint32_t& nCount);
    bool ReadZXLYNCount(uint32_t& nCount);
