...
bool CXlynStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CXLYNStake from zerocoin spend", __func__);
...
bool CXlynStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CXlynStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CXlynStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CXlynStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CXlynStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CXlynStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CXlynStake::GetUniqueness() const
{
    //The unique identifier for a XLYN stake is the outpoint
...
CBlockIndex* CXlynStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CXlynStake::ContextCheck(int nHeight, uint32_t nTime)
{
