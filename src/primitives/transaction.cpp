...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between XLYN and zXLYN
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // XLYN: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
