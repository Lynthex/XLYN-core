...
{
    CDataStream serializedCoinSpend = ZXLYNModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZXLYNSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zXLYN supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zXLYN supply
    ...
    // A one-time event where only the zXLYN supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZXLYNSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zXLYN supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zXLYN supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
