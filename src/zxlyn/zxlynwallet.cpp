...
CzXLYNWallet::CzXLYNWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zXLYN seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXLYNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXLYNSeed_deprecated()) {
                    LogPrintf("%s: Updated zXLYN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zXLYN seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zXLYN wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXLYNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXLYNCount(nCountLastUsed);
    else if (!walletdb.ReadZXLYNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXLYNWallet::Lock()
{
    seedMaster.SetNull();
}

void CzXLYNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXLYNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXLYN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXLYNWallet::LoadMintPoolFromDB()
{
...
void CzXLYNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXLYNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXLYNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zxlynTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXLYNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXLYN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zXLYNTracker which also adds to database
    wallet->zxlynTracker->Add(dMint, true);
...
void CzXLYNWallet::SeedToZXLYN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzXLYNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXLYNWallet::GenerateDeterministicZXLYN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXLYNWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXLYNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzXLYNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
