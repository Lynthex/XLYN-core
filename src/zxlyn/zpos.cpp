...
bool CLegacyZXlynStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZXLYNStake from non zc-spend", __func__);
    ...
    *this = CLegacyZXlynStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zXLYN stake origin", __func__);

    // All good
    return true;
}

CLegacyZXlynStake::CLegacyZXlynStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZXlynStake::GetIndexFrom()
{
...
CAmount CLegacyZXlynStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZXlynStake::GetUniqueness() const
{
...
bool CLegacyZXlynStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zXLYN stake block: height %d outside range", __func__, nHeight);
