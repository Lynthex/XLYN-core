...
#include "zxlyn/zxlynwallet.h"


CzXLYNTracker::CzXLYNTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzXLYNTracker::~CzXLYNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzXLYNTracker::Init()
{
...
bool CzXLYNTracker::Archive(CMintMeta& meta)
{
...
bool CzXLYNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzXLYNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzXLYNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzXLYNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzXLYNTracker::GetSerialHashes()
{
...
CAmount CzXLYNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzXLYNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzXLYNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzXLYNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzXLYNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzXLYNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzXLYNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzXLYNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzXLYNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzXLYNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzXLYNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzXLYNWallet* zXLYNWallet)
{
    bool iszXLYNWalletInitialized = (nullptr != zXLYNWallet);
    CMintMeta meta;
    ...
    if (!iszXLYNWalletInitialized)
        zXLYNWallet = new CzXLYNWallet(wallet);
    meta.isSeedCorrect = zXLYNWallet->CheckSeed(dMint);
    if (!iszXLYNWalletInitialized)
        delete zXLYNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzXLYNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXLYNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzXLYNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzXLYNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzXLYNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzXLYNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzxlyn from DB\n", __func__, listDeterministicDB.size());
    }
