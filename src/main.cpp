...
#if defined(NDEBUG)
#error "LYNTHEX cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uXLYN) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zXLYN mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zXLYN mint output in tx %s",
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZXLYNModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    //Track zXLYN money supply
    if (!UpdateZXLYNSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zXLYN supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("lynthex-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZXLYNModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
    ...
    // Update zXLYN money supply map
    if (!UpdateZXLYNSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zXLYN supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zXLYN supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateXLYNSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zXLYN spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zXLYN serial %s in block\n Block: %s",
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> xlynInputs;
        std::vector<CTxIn> zXLYNInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zXLYNInputs.push_back(stakeIn);
            }else{
                xlynInputs.push_back(stakeIn);
            }
        }
        const bool hasXLYNInputs = !xlynInputs.empty();
        const bool hasZXLYNInputs = !zXLYNInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasXLYNInputs) {
                    ...
                    for (const CTxIn& xlynIn : xlynInputs)
                        if(xlynIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zXLYN inputs.
            if(hasZXLYNInputs) {
                for (const CTxIn& zXlynInput : zXLYNInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zXlynInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zXLYN");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // LYNTHEX: We use certain sporks during IBD, so check to see if they are
