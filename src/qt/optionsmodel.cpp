...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::XLYN);
    ...
    if (settings.contains("nAnonymizeLynthexAmount") || reset)
        SoftSetArg("-anonymizelynthexamount", settings.value("nAnonymizeLynthexAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in lynthex.cpp during shotdown to also remove the window positions
