...
#include "qt/lynthex/topbar.h"
#include "askpassphrasedialog.h"
#include "qt/lynthex/forms/ui_topbar.h"
#include "qt/lynthex/loadingdialog.h"
#include "qt/lynthex/lockunlock.h"
#include "qt/lynthex/qtutils.h"
#include "qt/lynthex/receivedialog.h"
...
TopBar::TopBar(LYNTHEXGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                        ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelAmountTopXlyn}, "amount-small-topbar");
    setCssProperty({ui->labelAmountXlyn}, "amount-topbar");
    setCssProperty({ui->labelPendingXlyn, ui->labelImmatureXlyn, ui->labelAvailableXlyn,
                       ui->labelLockedXlyn, ui->labelMasternodeCount, ui->labelCollateralXlyn,
    ...
    ui->pushButtonConf->setButtonText("lynthex.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open lynthex.conf with default application"));
}
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralXlyn->setText(GUIUtil::formatBalance(CMasternode::GetMasternodeNodeCollateral(chainActive.Tip()->nHeight), nDisplayUnit));
}
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // XLYN
    // Top
    ui->labelAmountTopXlyn->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    // Expanded
    ui->labelAmountXlyn->setText(GUIUtil::formatBalance(newBalance.balance + newBalance.immature_balance, nDisplayUnit));
    ui->labelAvailableXlyn->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    ui->labelPendingXlyn->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureXlyn->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
    ui->labelLockedXlyn->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
