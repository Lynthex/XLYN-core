...
#include "qt/lynthex/dashboardwidget.h"
#include "qt/lynthex/forms/ui_dashboardwidget.h"
#include "qt/lynthex/sendconfirmdialog.h"
#include "qt/lynthex/txrow.h"
...
DashboardWidget::DashboardWidget(LYNTHEXGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of XLYN earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareXlyn, "square-chart-xlyn");
    setCssProperty(ui->labelXlyn, "text-chart-xlyn");
    ...
    setCssProperty(ui->labelAmountXlyn, "text-stake-xlyn-disable");
	ui->labelAmountMNRewards->setText("0 XLYN");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &LYNTHEXGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &LYNTHEXGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &LYNTHEXGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("XLYN")
    updateDisplayUnit();
}
...
const QMap<int, QMap<QString, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isXlyn = modelIndex.data(TransactionTableModel::TypeRole).toInt() == TransactionRecord::StakeMint;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair XLYN, zXLYN
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            xlyn = (pair["xlyn"] != 0) ? pair["xlyn"] / 100000000 : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / 100000000 : 0;
            chartData->totalXlyn += pair["xlyn"];
            chartData->totalMNRewards += pair["mn"];
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesXlyn.append(xlyn);    
        chartData->valuesMNRewards.append(mnrewards);    

        int max = std::max(xlyn, mnrewards);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesXlyn);
    set1->append(chartData->valuesMNRewards);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalXlyn > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountXlyn, "text-stake-xlyn");
	...
	forceUpdateStyle({ui->labelAmountXlyn, ui->labelAmountMNRewards});
    ui->labelAmountXlyn->setText(GUIUtil::formatBalance(chartData->totalXlyn, nDisplayUnit));
