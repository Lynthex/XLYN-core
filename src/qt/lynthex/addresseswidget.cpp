...
#include "qt/lynthex/forms/ui_addresseswidget.h"
#include "qt/lynthex/addresslabelrow.h"
#include "qt/lynthex/addnewaddressdialog.h"
#include "qt/lynthex/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(LYNTHEXGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination xlynAdd = DecodeDestination(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(
            xlynAdd, label.toUtf8().constData(),AddressBook::AddressBookPurpose::SEND
        )) {
