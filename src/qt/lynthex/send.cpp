...
#include "qt/lynthex/forms/ui_send.h"
#include "qt/lynthex/addnewcontactdialog.h"
#include "qt/lynthex/qtutils.h"
#include "qt/lynthex/sendchangeaddressdialog.h"
#include "qt/lynthex/optionbutton.h"
#include "qt/lynthex/sendconfirmdialog.h"
#include "qt/lynthex/myaddressrow.h"
#include "qt/lynthex/guitransactionsutils.h"
...
SendWidget::SendWidget(LYNTHEXGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-xlyn");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination xlynAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(xlynAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(xlynAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
