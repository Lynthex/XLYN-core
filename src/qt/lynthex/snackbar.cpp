...
#include "qt/lynthex/forms/ui_snackbar.h"
#include "qt/lynthex/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(LYNTHEXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &LYNTHEXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
