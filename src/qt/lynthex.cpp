...
#include "config/lynthex-config.h"
#endif
...
#include "qt/lynthex/splash.h"
#include "qt/lynthex/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("lynthex-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in lynthex.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in lynthex.qrc)
...
/** Class encapsulating LYNTHEX Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (LYNTHEXGUI)
...
{
    window = new LYNTHEXGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &LYNTHEXGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &LYNTHEXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(LYNTHEXGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(LYNTHEXGUI::DEFAULT_WALLET);
        ...
        // LYNTHEX: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &LYNTHEXGUI::handlePaymentRequest);
        connect(window, &LYNTHEXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. LYNTHEX can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(lynthex_locale);
    Q_INIT_RESOURCE(lynthex);
    ...
    /// 6. Determine availability of data directory and parse lynthex.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("LYNTHEX"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("LYNTHEX"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // lynthex: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("LYNTHEX didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
