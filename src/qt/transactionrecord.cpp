        ...
        QList<TransactionRecord>& parts)
{
    ...
    if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zxlynTracker->HasMintTx(hash))) {
        //zXLYN stake reward
        sub.involvesWatchAddress = false;
        sub.type = TransactionRecord::StakeZXLYN;
                                           ...
                                           QList<TransactionRecord>& parts)
{
    ...
    for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
        ...
        if (txout.IsZerocoinMint()) {
            ...
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            sub.type = TransactionRecord::ZerocoinSpend_Change_zXlyn;
            sub.address = getValueOrReturnEmpty(wtx.mapValue, "zerocoinmint");
            if (!fFeeAssigned) {
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZXLYNModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
...
bool IsZXLYNType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZXLYN:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zXlyn:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZXLYN ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZXLYN);
}
