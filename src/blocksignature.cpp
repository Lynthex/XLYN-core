...
bool CheckBlockSignature(const CBlock& block, const bool enableP2PKH)
{
    ...
    /** Each block is signed by the private key of the input that is staked. This can be either zXLYN or normal UTXO
     *  zXLYN: Each zXLYN has a keypair associated with it. The serial number is a hash of the public key.
     *  UTXO: The public key that signs must match the public key associated with the first utxo of the coinstake tx.
     */
    CPubKey pubkey;
    bool fzXLYNStake = block.vtx[1].vin[0].IsZerocoinSpend();
    if (fzXLYNStake) {
