...
void PrepareShutdown()
{
    ...
    util::ThreadRename("lynthex-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), LYNTHEX_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), LYNTHEX_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all XLYN specific functionality (Masternodes, Zerocoin, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), LYNTHEX_MASTERNODE_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:18444"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The LYNTHEX Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("lynthex-loadblk");
...
/** Sanity checks
 *  Ensure that LYNTHEX is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. LYNTHEX is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single LYNTHEX process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. LYNTHEX is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //LYNTHEX specific: zerocoin and spork DB's
                ...
                // LYNTHEX: load previous sessions sporks if we have them.
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zXLYN if already reindexed
                    RecalculateXLYNSupply(1, fReindexZerocoin);
                }
