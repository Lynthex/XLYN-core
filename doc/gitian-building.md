...
*Setup instructions for a gitian build of LYNTHEX Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the LYNTHEX
...
to the LYNTHEX GitHub release page.
...
- [Building LYNTHEX Core](#building-lynthex-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/lynthex-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/lynthex-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/lynthex-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/lynthex-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/lynthex-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/lynthex-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/lynthex-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@lynthex.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/lynthex-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/lynthex-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/lynthex-osx-*-build.assert
```
