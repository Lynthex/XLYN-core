UNIX BUILD NOTES
====================
Some notes on how to build LYNTHEX Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile LYNTHEX Core and the dependencies,
...
This will build lynthex-qt as well, if the dependencies are met.
...
memory available when compiling LYNTHEX Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~lynthex/+archive/lynthex).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:lynthex/lynthex
    sudo apt-get update
...
To build LYNTHEX Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build lynthex-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a lynthex-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip lynthexd" to strip the debug
...
Security
--------
To help make your LYNTHEX Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, LYNTHEX Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, LYNTHEX Core may be compiled in
