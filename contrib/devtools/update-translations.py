...
def remove_current_translations():
    ...
    This leaves lynthex_en.ts untouched.
    '''
...
def update_build_systems():
    ...
    '''
    filename_lang = [re.match(r'((lynthex_(.*)).ts)$', filename).groups() for (filename, filepath) in all_ts_files(include_source=True)]
    filename_lang.sort(key=lambda x: x[0])

    # update qrc locales
    with open('src/qt/lynthex_locale.qrc', 'w') as f:
